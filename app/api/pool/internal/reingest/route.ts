...
/** Busy playoff slates + DB writes; Vercel project max duration must allow this value. */
export const maxDuration = 120;

/**
 * Manual pool ingest: same behavior as cron + `ingest-daily`, with shared auth.
 *
 * - **GET** — ingest pool-calendar **yesterday** only (same as `/api/pool/internal/cron-ingest-yesterday`; run `/api/pool/internal/cron-materialize-yesterday` for snapshots).
